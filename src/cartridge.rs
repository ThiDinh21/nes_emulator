...
impl Rom {
    pub fn new(raw: &Vec<u8>) -> Result<Rom, String> {
        ...
        if ines_ver != 0 {
            return Err("iNES 2.0 format is not supperted".to_string());
        }
        ...
        let trainer_size = if (raw[6] >> 2) & 0b1 == 1 { 512 } else { 0 };
        let prg_rom_start = 16 + trainer_size;
        let chr_rom_start = 16 + trainer_size + prg_rom_size;
...
mod test {
    ...
    fn test_nes2_is_not_supported() {
        ...
        match rom {
            ...
            Result::Err(str) => assert_eq!(str, "iNES 2.0 format is not supperted"),
        }
    }
}
